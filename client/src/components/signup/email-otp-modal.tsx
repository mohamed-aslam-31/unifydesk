...
interface EmailOtpModalProps {
  ...
  onResendUpdate?: (email: string, count: number) => void;
  onAttemptUpdate?: (email: string, count: number) => void;
  onCooldownUpdate?: (email: string, seconds: number) => void;
  onEmailBlocked?: (email: string) => void;
}
...
export function EmailOtpModal({ 
  ...
  useEffect(() => {
    if (onCooldownUpdate && cooldownTime >= 0) {
      const timeoutId = setTimeout(() => {
        onCooldownUpdate(email, cooldownTime);
      }, 0);
      return () => clearTimeout(timeoutId);
    }
  }, [cooldownTime, email, onCooldownUpdate]);
  ...
  const handleSendOtp = async (isResendAction = false) => {
    ...
    if (resendCount >= 3) {
      toast({
        ...
        description: "Your email has reached the maximum of 3 resend attempts.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        if (data.remainingCooldown) {
          ...
          setTimeout(() => {
            if (onCooldownUpdate) {
              onCooldownUpdate(email, newCooldown);
            }
          }, 0);
          toast({
            title: "Please wait",
        ...
        if (response.status === 429) {
          if (data.message.includes("Maximum resend attempts")) {
            toast({
              title: "Resend Limit Reached",
              description: "Your email has reached the maximum resend limit. Please try again later.",
              variant: "destructive",
            });
            return;
          }
          ...
          if (data.message.includes("Blocked")) {
            setIsBlocked(true);
            // Notify parent that this email is blocked
            setTimeout(() => {
              if (onEmailBlocked) {
                onEmailBlocked(email);
              }
            }, 0);
            toast({
              title: "Email Blocked",
              description: "Your email was blocked due to too many attempts. Please try again after 5 hours.",
              variant: "destructive",
            });
            return;
          }
      ...
      setTimeout(() => {
        if (onResendUpdate) {
          onResendUpdate(email, newCount);
        }
        if (onCooldownUpdate) {
          onCooldownUpdate(email, 180);
        }
        if (onOtpSent) {
          onOtpSent(email);
        }
  ...
  const handleSubmitOtp = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (onAttemptUpdate) {
          onAttemptUpdate(email, newAttempts);
        }
  ...
  // Update counts from props when email changes
  useEffect(() => {
    // Only clear OTP when email actually changes, not on every prop update
    setOtp('');
    setSessionId(null);
  ...
  }, [email]); // Only depend on email to avoid clearing during typing
